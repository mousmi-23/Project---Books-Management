...
const createReview = async function (req, res) {
    try {
        ...
        let book_id = bookId.trim() // here you can pass space
        ...
        if (paramsBookId === book_id) {
            ...
            if (review) { // not accept only quote but store in schema have to handle comming soon
            ...
            // here we not populate with book data can see only review model comming soon
        }
    } catch (err) {
...
const updatedReview = async function (req, res) {
    try {
        ...
        if (!isReviewPresent) return res.status(400).send({ status: true, msg: "Review is not present may be book is deleted" })
        // we can also handle indivisual    comming soon
        ...
        if (!isValidRequestBody(data)) return res.status(400).send({ status: false, message: "Please provide Update Data in request body" })

        if (Object.keys(data).length != 0) {
            let emptyInput = filters.filter((ele) => ele[1] == '')
            if (emptyInput.length != 0) {
                return res.status(400).send({ status: false, msg: "You can not pass empty key value" })
            }
        }
